...
int genAST(struct ASTnode * n, int reg, int parentASTop){
  ...
  if (n-> left)
    leftreg = genAST(n-> left,NOREG, n->op); // we first evalue left tree 
  if (n-> right)
    rightreg = genAST(n-> right, leftreg,n->op); // we are passing the result of left tree
  ...
  switch (n-> op){
    ...
    case A_ASSIGN:
      ...
      return(rightreg);
    case A_PRINT: 
      // Print the left-childs value and return no register 
      genprintint(leftreg);
      genfreeregs();
      return (NOREG);
    default: 
