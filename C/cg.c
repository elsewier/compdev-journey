...
static char  *reglist[4] = { "%r8", "%r9", "%r10", "%r11"};     // 64 bit 
static char *breglist[4] = { "%r8b", "%r9b", "%r10b", "%r11b"}; // lower 8bit part 
...
static int cgcompare(int r1, int r2, char *how){
  ...
  fprintf(Outfile, "\t%s\t%s\n", how, breglist[r2]); // set to 1 
  fprintf(Outfile, "\tandq\t$255,%s\n", reglist[r2]); // remove all other bits using and comparison 
  free_register(r1);
  return(r2);
}

int cgequal(int r1, int r2) {
  return (cgcompare(r1, r2, "sete")); // if its equal sete flag is set 
}
